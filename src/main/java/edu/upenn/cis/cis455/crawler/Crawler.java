...
public class Crawler implements CrawlMaster {
    ...
    public Crawler(String url, int size, int count) {
        configureTopology(4, 4, 4);
        SharedInfo.getInstance().setMaster(this);
        SharedInfo.getInstance().addUrl(url);
        SharedInfo.getInstance().setMaxContentLength(size);
        SharedInfo.getInstance().setMaxDocumentCount(count);
    }
